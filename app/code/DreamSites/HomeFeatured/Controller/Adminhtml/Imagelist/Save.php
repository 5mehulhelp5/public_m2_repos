...
class Save extends Action
{
     ...
     * Image factory
     *
     * @var \DreamSites\HomeFeatured\Model\ImageFactory
     */
    ...
    public function __construct(
        ...
        \DreamSites\HomeFeatured\Model\ImageFactory       $imageFactory,
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (($_FILES['image_filename']['name'] !== '') && isset($_FILES['image_filename']['name'])) {
                try {
                    $directory = $this->_dir->getPath('media') . '/featured/';
            ...
            if (empty($data['heading_colour'])) {
                $data['heading_colour'] = '#ee2761';
            }

            if (empty($data['title_colour'])) {
                $data['title_colour'] = '#061738';
            }

            if (empty($data['link_colour'])) {
                $data['link_colour'] = '#606060';
            }

            $pageData->setData($data)->save();

            $this->messageManager->addSuccess(__('Saved Featured Item.'));
            $resultRedirect->setPath('homefeatured/imagelist');
        } catch (\Exception $e) {
            $this->messageManager->addError($e->getMessage());
            $this->_getSession()->setPageData($data);
            // Log the error using Magento's PSR-3 logger
            $this->_logger->error('Save Featured Item failed: ' . $e->getMessage(), [
                'exception' => $e,
                'file' => __FILE__,
                'line' => __LINE__
            ]);

            $resultRedirect->setPath('homefeatured/imagelist/edit', ['image_id' => $id]);
        }
